...
2. 文献综述素材（`02_Literature/`，特别是 `04_gap_analysis.md` 和 `03_comparison_table.md`）
...
2. **Introduction（引言）**
   ...
   - 简述深度学习在医学影像中的发展，并指出在氟斑牙/氟骨症方面的研究缺口（引用 `02_Literature/04_gap_analysis.md`）。
   - 明确本文贡献（通常 3-4 点），与 InnoAgent 的创新点对齐。

3. **Related Work（相关工作）**
   - 分 2-3 个子节：如“氟中毒影像诊断”、“深度学习在牙科/骨科的应用”、“多模态融合与可解释性”。
...
8. **参考文献**
   - 使用 `08_Manuscript/bibliography.bib`，确保所有引用在正文中出现，格式符合期刊要求。
