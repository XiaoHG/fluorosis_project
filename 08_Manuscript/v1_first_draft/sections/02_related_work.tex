...
EDL has been applied to skin lesion classification, histopathology image analysis, and COVID-19 diagnosis from chest X-rays \cite{ulmer2022prior}. Recent extensions include mutual EDL for semi-supervised segmentation \cite{he2024medl}, uncertainty-aware evidential fusion \cite{zheng2023uncertaintyfusion}, and reliability-aware discounting for noisy labels \cite{li2024rednet}, demonstrating EDL's versatility across medical imaging tasks. However, EDL has not been previously applied to fluorosis diagnosis, nor has it been combined with ordinal regression constraints in a shared-logit architecture. Our work is the first to demonstrate EDL's effectiveness for dental fluorosis grading, including its regularization benefits for small-sample initialization robustness.

\subsection{Ordinal Regression and Calibration}

Ordinal regression addresses classification with ordered labels, where the cost of misclassification depends on the distance between predicted and true classes \cite{cao2020corn}. Standard deep learning approaches include threshold-based cumulative link models, CORN/CORAL frameworks \cite{cao2020corn}, and soft-encoded ordinal regression (SORD) \cite{diaz2018sord} which replaces one-hot labels with soft target distributions.
